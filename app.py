...
def plotting(data):
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=data.index,y=data['Volume'],name='Stock Open'))
    fig.layout.update(title_text=f'Volume of {stocks_name} Stock',xaxis_rangeslider_visible=True)
    st.plotly_chart(fig)
plotting(data)
